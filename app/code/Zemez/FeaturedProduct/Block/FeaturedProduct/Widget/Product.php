...
namespace Zemez\FeaturedProduct\Block\FeaturedProduct\Widget;
...
class Product extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
    ...
    protected $_template = 'Zemez_FeaturedProduct::widget/products-grid.phtml';
    ...
    public function __construct(
        ...
        \Zemez\FeaturedProduct\Model\ProductFactory $productFactory,
...
//        $productIdsStr = false;
    ...
    public function truncateProductName($name)
    {
        ...
        if($truncate != 0 && is_int($truncate) && $nameLength > $truncate){
            $name = iconv_substr($name, 0, $truncate) . '...';
        }
        return $name;
    }
    ...
    public function getImage($product, $imageId, $attributes = [])
    {
        ...
        if($this->getProductImageWidth() && $this->getProductImageHeight()) {
            $imageSize = [
            ...
            $result->setWidth($this->getProductImageWidth());
            $result->setHeight($this->getProductImageHeight());
            $result->setRatio((float) $this->getProductImageHeight() / $this->getProductImageWidth());
        }
        ...
        switch ($hoverType) {
            case ('switcher'):
                 ...
                 */
                $result->setTemplate('Zemez_FeaturedProduct::product/switcher.phtml');
                ...
                break;
            case ('carousel'):
                /** @var $result \Zemez\FeaturedProduct\Block\Product\View\Gallery */
                $result = $this->_layout->createBlock('Zemez\FeaturedProduct\Block\Product\View\Gallery')
                    ->setTemplate('Zemez_FeaturedProduct::product/carousel.phtml');
            ...
            case ('gallery'):
                /** @var $result \Zemez\FeaturedProduct\Block\Product\View\Gallery */
                $result = $this->_layout->createBlock('Zemez\FeaturedProduct\Block\Product\View\Gallery')
                    ->setTemplate('Zemez_FeaturedProduct::product/gallery.phtml');
                $this->_setupGallery($product, $result);
                $result->setSlidesCount($this->getGallerySlidesCount())
                       ->setThumbnailSize(['width' => $this->getThumbWidth() ?? $result->getImageAttribute('product_page_image_small', 'width'), 'height' => $this->getThumbHeight() ?? $result->getImageAttribute('product_page_image_small', 'height')])
                       ->setSlideSize($imageSize);
                break;
            default: break;
        }
