...
namespace AJH\ProductVehicle\Model\ResourceModel\Makes;

use AJH\ProductVehicle\Model\ResourceModel\Makes as ResourceModel;
use AJH\ProductVehicle\Model\Makes as Model;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection {
    ...
    public function addFilterByPartNumber($partNumber) {
        ...
        $this->getSelect()->joinInner(
                array('models' => $modelsTable), 'models.MakesID = main_table.ID', array('MakesID')
        );
        // join table to application guide
        $this->getSelect()->joinInner(
                array('applicationguide' => $applicationGuideTable), 'applicationguide.ModelsID = models.ID', array('ModelsID')
        );
        // join table to parts table
        $this->getSelect()->joinInner(
                array('partmaster' => $partsTable), 'partmaster.PartClassID=applicationguide.PartClassID', array('PartNumber')
        );
