...
class CategoryCollection extends \Magento\Framework\View\Element\Template
{
     ...
     * @return string
     */
    public function getAllCategories($category, $categoryDisable, $level=1)
    {
        ...
        if ($childrenCategories = $category->getChildrenCategories()) {
            foreach ($childrenCategories as $category) {
                ...
                if ($categoryValidate == false) {
                    ...
                    $categoryHtml = '<li><a href="' . $categoryUrl . '">' . $category->getName() . '</a>';
                    $categoryReturn = $this->getAllCategories($category, $categoryDisable, $category->getLevel());
                    $categoryHtml = $categoryHtml . $categoryReturn . '</li>';
                } else {
                    $categoryHtml = null;
                }
            ...
            if(trim($categoryHtmlEnd)!==''){
                return '<ul class="sitemap-category-list category-level-'.$level.'">' . $categoryHtmlEnd . '</ul>';
            }else{
                return '';
            }
        }
